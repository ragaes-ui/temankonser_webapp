    ...
    view: () => {
      ...
      return m("div", { class: `min-h-screen flex flex-col ${bgRoot} font-sans transition-colors duration-300`, key: "layout" },
        ...
        m("main", { class: "container mx-auto p-4 md:p-8 flex-grow" },
          isLoading ? 
            m("div", { class: "flex flex-col items-center justify-center mt-32 animate-[pulse_0.5s_ease-out_infinite]" },
              m("div", { class: `w-12 h-12 border-4 ${isDark ? 'border-zinc-800' : 'border-gray-300'} border-t-indigo-500 rounded-full animate-spin mb-4 shadow-lg` }),
          ...
          :
            m("div", { class: "animate-[fadeIn_0.3s_ease-out_1]" },
              currentId === "home" ? 
                ...
                m("div", { class: "flex flex-col gap-20 mt-12 pb-16 items-center text-center" },
                  m("div", { class: "max-w-3xl mx-auto flex flex-col items-center gap-6" },
                    ...
                    m("div", { class: `w-16 h-1 ${isDark ? 'bg-zinc-800' : 'bg-gray-300'} rounded-full my-2` }),
                    m("p", { class: `text-md ${isDark ? 'text-zinc-500' : 'text-gray-500'}` }, t.hint)
                  ),
                    ...
                    m("div", { class: "grid grid-cols-1 md:grid-cols-3 gap-6" },
                      t.stats.map((stat, index) => 
                        m("div", { 
                          ...
                          m("p", { class: `font-medium text-lg ${isDark ? 'text-zinc-200' : 'text-black'}` }, stat.t),
                          m("p", { class: `text-sm mt-2 ${isDark ? 'text-zinc-400' : 'text-gray-500'}` }, stat.d)
                        )
                      )
                    )
                  )
              ...
              : 
                ...
                (activeConcert ? 
                  m("div", { class: `${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-gray-200'} rounded-xl border p-8 mb-10 text-center max-w-5xl mx-auto mt-4 shadow-xl transition-colors` },
                    ...
                    : 
                      m("div", { class: "mt-12 animate-[fadeIn_0.5s_ease-out_1]" },
                        m("div", { class: "grid grid-cols-1 lg:grid-cols-2 gap-10 items-start text-left" },
                          ...
                          m("div", { class: "w-full" },
                            m("div", { class: `flex items-center gap-3 mb-6 border-b ${isDark ? 'border-zinc-800' : 'border-gray-200'} pb-3` },
                              m("span", { class: "text-2xl" }, "📷"),
                              m("h3", { class: `text-2xl font-bold ${textHeading}` }, t.photoArc)
                            ),
                            m("div", { class: `${isDark ? 'bg-black border-zinc-800' : 'bg-gray-50 border-gray-200'} p-4 rounded-2xl border shadow-inner` },
                          ...
                          hasVideos ? 
                            m("div", { class: "w-full" },
                              ...
                              m("div", { class: `grid grid-cols-1 sm:grid-cols-2 gap-4 ${isDark ? 'bg-black border-zinc-800' : 'bg-gray-50 border-gray-200'} p-4 rounded-2xl border shadow-inner` },
                                (activeConcert.videos || []).map((vidUrl, index) => 
                                  m("div", { class: `w-full ${isDark ? 'bg-zinc-950' : 'bg-black'} p-2 rounded-xl shadow-lg border border-zinc-800 animate-[fadeIn_0.5s_ease-out_1]` },
                          ...
                          : 
                            m("div", { class: `w-full flex flex-col items-center justify-center ${isDark ? 'bg-zinc-900/50 border-zinc-800' : 'bg-gray-50 border-gray-300'} p-10 rounded-2xl border border-dashed` },
                              m("span", { class: "text-4xl mb-3 opacity-50" }, "🎥"),
                              m("p", { class: `${isDark ? 'text-zinc-500' : 'text-gray-500'} italic text-center` }, t.noVideo)
                            )
                        )
                      )
                  )
        ...
        m("div", { class: "fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans" },
          isChatOpen ? m("div", { class: `${isDark ? 'bg-zinc-900 border-zinc-700' : 'bg-white border-gray-300'} border rounded-2xl w-80 shadow-[0_0_30px_rgba(0,0,0,0.5)] mb-4 overflow-hidden flex flex-col animate-[fadeIn_0.2s_ease-out_1]` },
            m("div", { class: "bg-indigo-600 p-4 flex justify-between items-center text-white" },
              ...
              m("button", { class: "hover:text-zinc-300 font-bold", onclick: () => isChatOpen = false }, "✕")
            ),
            m("div", { class: `p-4 h-72 overflow-y-auto flex flex-col gap-3 ${isDark ? 'bg-black' : 'bg-gray-50'} text-sm`, id: "chat-box", onupdate: (vnode) => vnode.dom.scrollTop = vnode.dom.scrollHeight },
              chatHistory.map(chat => 
                m("div", { class: `p-3 max-w-[85%] rounded-xl shadow-md ${chat.role === 'ai' ? (isDark ? 'bg-zinc-800 text-zinc-200' : 'bg-gray-200 text-black') + ' self-start rounded-tl-none' : 'bg-indigo-500 text-white self-end rounded-tr-none'}` }, 
                  chat.text
                )
              ),
              isAiTyping ? m("div", { class: `${textMuted} italic text-xs ml-2` }, Settings.lang === "id" ? "AI sedang mengetik..." : "AI is typing...") : null
            ),
            m("form", { class: `flex p-3 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-gray-200'} border-t`, onsubmit: sendChatMessage },
              m("input", { class: `flex-grow ${isDark ? 'bg-black border-zinc-700 text-white' : 'bg-gray-50 border-gray-300 text-black'} border rounded-lg p-2 outline-none focus:border-indigo-500 text-sm`, placeholder: Settings.lang === "id" ? "Tanya apa aja..." : "Ask me anything...", value: chatMessage, oninput: e => chatMessage = e.target.value }),
        ...
        // --- FOOTER SOSIAL MEDIA ---
        m("footer", { class: `w-full ${isDark ? 'bg-black border-zinc-900' : 'bg-white border-gray-200'} border-t py-8 mt-12 text-center transition-colors` },
          m("div", { class: "container mx-auto" },
            ...
            m("div", { class: "flex justify-center items-center gap-6 mb-6" },
              m("a", { href: "https://www.instagram.com/temankonser.fest", target: "_blank", class: `${isDark ? 'text-zinc-300' : 'text-gray-600'} hover:text-indigo-500 font-medium transition-colors` }, "Instagram"),
              m("a", { href: "https://twitter.com/", target: "_blank", class: `${isDark ? 'text-zinc-300' : 'text-gray-600'} hover:text-indigo-500 font-medium transition-colors` }, "Twitter"),
              m("a", { href: "https://www.tiktok.com/@temankonser.fest", target: "_blank", class: `${isDark ? 'text-zinc-300' : 'text-gray-600'} hover:text-indigo-500 font-medium transition-colors` }, "Tiktok")
            ),
            m("p", { class: `${isDark ? 'text-zinc-500' : 'text-gray-400'} text-sm` }, "© 2026 TemanKonser.fest. Created By ragaes-ui.")
          )
        )
      );
    }
